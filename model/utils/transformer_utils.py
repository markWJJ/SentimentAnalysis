...
def transformer_encoder(encoder_emb, name, context_mask,args,dropout,reuse=False, num_blocks=4):
    with tf.variable_scope(name_or_scope=name, reuse=reuse):
        data = []
        for i in range(num_blocks):
            with tf.variable_scope("num_blocks_{}".format(i)):
                ### Multihead Attention
                # encoder_emb=tf.multiply(encoder_emb,tf.cast(tf.expand_dims(context_mask,2),tf.float32))
                encoder_emb = multihead_attention(queries=encoder_emb,
                                                   keys=encoder_emb,
                                                   scope="multihead_attention_{}".format(i),
                                                   num_units=args.emb_size,
                ...
                ### Feed Forward
                encoder_emb = feedforward(encoder_emb,
                                           num_units=[args.num_heads * args.emb_size, args.emb_size])
                data.append(encoder_emb)
        return data[-1]
...
def build_transformer_emb(sent_word,sent_word_emb,text_len,args, dropout,name,reuse=False):
        with tf.variable_scope(name_or_scope=name,reuse=reuse):
            encoder_input_emb=sent_word_emb
            if args.sinusoid:
...
def transformer_encoder_ht(inputs, target_space, hparams, features=None, losses=None):
